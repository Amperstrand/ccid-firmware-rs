...
/// Configurable smartcard timing and clock parameters (ISO 7816-3).
#[derive(Debug, Clone, Copy)]
pub struct SmartcardConfig {
    /// Power-on delay after asserting PWR (card supply stable).
    ...
    pub atr_post_rst_delay_ms: u32,
    /// Delay after CLK on before RST high (ISO 7816-3: min 40k clock cycles
    /// after CLK before RST high; ~11ms at 3.57MHz).
    pub clk_to_rst_delay_ms: u32,
    /// Overall ATR timeout (waiting for first byte from card).
    pub atr_timeout_ms: u32,
    /// Per-byte timeout during ATR; longer than byte_timeout_ms
    /// (ISO 7816-3 initial character delay up to ~9600 ETU).
    pub atr_byte_timeout_ms: u32,
    /// Per-byte timeout during normal T=0/T=1 exchanges.
    pub byte_timeout_ms: u32,
    /// Procedure-byte timeout (waiting for card acknowledgement).
    pub procedure_timeout_ms: u32,
    /// Default ETU divisor (Fi=372 at Di=1, per ISO 7816-3 default TA1).
    pub default_etu: u32,
    /// Maximum smartcard clock frequency in Hz (5 MHz = ISO 7816 maximum).
    pub max_clk_hz: u32,
}
...
pub const SMARTCARD_CONFIG_DEFAULT: SmartcardConfig = SmartcardConfig {
    ...
    reset_delay_ms: 25,
    atr_post_rst_delay_ms: 20,
    clk_to_rst_delay_ms: 15,
    atr_timeout_ms: 400,
    atr_byte_timeout_ms: 1000,
    byte_timeout_ms: 200,
    procedure_timeout_ms: 5000,
    default_etu: 372,
    max_clk_hz: 5_000_000,
};
...
mod tests {
    ...
    #[test]
    fn smartcard_config_default_matches_historical_consts() {
        assert_eq!(SMARTCARD_CONFIG_DEFAULT.power_on_delay_ms, 50);
        assert_eq!(SMARTCARD_CONFIG_DEFAULT.reset_delay_ms, 25);
        assert_eq!(SMARTCARD_CONFIG_DEFAULT.atr_post_rst_delay_ms, 20);
        assert_eq!(SMARTCARD_CONFIG_DEFAULT.clk_to_rst_delay_ms, 15);
        assert_eq!(SMARTCARD_CONFIG_DEFAULT.atr_timeout_ms, 400);
        assert_eq!(SMARTCARD_CONFIG_DEFAULT.atr_byte_timeout_ms, 1000);
        assert_eq!(SMARTCARD_CONFIG_DEFAULT.byte_timeout_ms, 200);
                        ...
                        if pb == INS_GET_RESPONSE || pb == 0x4F {
                            ...
                            for i in 0..n {
                                response[response_len + i] =
                                    io.recv_byte_timeout(byte_timeout_ms)?;
                            }
                            response_len += n;
