...
impl<E, COMM: Interface<Error = E>> Mfrc522<COMM, Uninitialized> {
    ...
    pub fn init(mut self) -> Result<Mfrc522<COMM, Initialized>, Error<E>> {
        ...
        //   for a ~100ms timeout, we need a value of 4000 = 0x0FA0
        //   Ref: Arduino MFRC522 library PCD_ConfigureDefaults() uses 0x0FA0
        //   Original upstream used 0x3E8 (25ms) which was too short for
        //   many cards to respond to REQA.
        self.write(Register::TReloadRegHigh, 0x0F)?;
        self.write(Register::TReloadRegLow, 0xA0)?;
...
impl<E, COMM: Interface<Error = E>> Mfrc522<COMM, Initialized> {
    ...
    pub fn rmw_register<F>(&mut self, reg: Register, f: F) -> Result<(), Error<E>>
    where
        F: FnOnce(u8) -> u8,
    {
        self.rmw(reg, f)
