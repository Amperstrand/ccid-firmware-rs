...
use ccid_firmware_rs::{
    ApduError, PinBuffer, PinEntryContext, PinEntryState, PinResult, PinVerifyParams,
    VerifyApduBuilder,
};
...
fn test_apdu_builder_non_ascii_digits() {
    ...
    for pin in invalid_pins {
        assert_eq!(
            ...
            "Expected InvalidPinCharacter for {:?}",
            pin
        );
    }
}
...
fn test_apdu_builder_build_from_digits() {
    ...
    // Invalid digit (> 9)
    assert_eq!(
        builder.build_from_digits(&[1, 2, 3, 4, 5, 10]),
        Err(ApduError::InvalidPinCharacter)
    );
}
...
// ============================================================================
// NOTE: These tests require the "display" feature
...
fn test_keypad_boundary_points() {
    ...
    assert_eq!(
        keypad.hit_test(Point::new(x_start + 1, y_start + 1)),
        ButtonId::Digit(5)
    );
