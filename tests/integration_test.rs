...
use ccid_firmware_rs::{
    PinBuffer, PinEntryContext, PinEntryState, PinResult, PinVerifyParams, VerifyApduBuilder,
};
...
fn test_apdu_validation() {
    ...
    // Too short (5 digits)
    assert_eq!(
        builder.build(b"12345"),
        Err(ccid_firmware_rs::ApduError::InvalidPinLength)
    );
    ...
    assert_eq!(
        ...
        Err(ccid_firmware_rs::ApduError::InvalidPinCharacter)
    );
